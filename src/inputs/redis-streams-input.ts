...
export const createRedisStreamsInput = (
  ...
  Effect.runSync(validate(RedisStreamsInputConfigSchema, config, "Redis Streams Input configuration").pipe(
    Effect.catchAll((error) =>
      Effect.fail(new RedisStreamsInputError(error.message, error.category, error))
    )
  ))
  ...
  const mode = config.mode ?? (config.consumerGroup ? "consumer-group" : "simple")
  ...
  if (mode === "simple") {
    ...
    const stream = Stream.repeatEffect(
      Effect.gen(function* () {
        ...
        yield* Effect.logInfo(`Connected to Redis stream: redis://${config.host}:${config.port}/${config.db || 0}`)
        yield* Effect.logDebug(`Polling Redis stream ${config.stream} from ${lastId}`)

        const [results, readDuration] = yield* measureDuration(Effect.tryPromise({
          try: async () =>
            await client.xread(
              "COUNT",
              count,
        ...
        yield* Effect.logDebug(`Read ${messages.length} messages from Redis stream`)
        return messages
      })
    ).pipe(
  ...
  const consumerName = config.consumerName || "consumer-" + Math.random().toString(36).slice(2)
  const metrics = new MetricsAccumulator("redis-streams-input")
  let messageCount = 0
  ...
  const ackMessage = (entryId: string): Effect.Effect<void, RedisStreamsInputError> =>
    Effect.tryPromise({
      try: async () => {
  ...
  const stream = Stream.repeatEffect(
    Effect.gen(function* () {
        ...
        (entry) =>
          Effect.gen(function* () {
            ...
            yield* ackMessage(entryId).pipe(
              Effect.catchAll((error) => {
                metrics.recordError()
                return Effect.logError(`Failed to ACK ${entryId}: ${error.message}`)
              })
            )
            return msg
          }),
      ...
      yield* Effect.logDebug(`Read and ACKed ${messages.length} messages from Redis stream`)
      return messages
    })
  ).pipe(
    Stream.flatMap(Stream.fromIterable),
