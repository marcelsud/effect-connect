 ...
 */
const serializeMessage = (msg: Message, delaySeconds?: number): { body: string; attributes: Record<string, any>; delay?: number } => ({
...
export const createSqsOutput = (
  ...
  Effect.runSync(validate(SqsOutputConfigSchema, config, "SQS Output configuration").pipe(
  ...
  const timeoutFiberRef = Ref.unsafeMake<Fiber.RuntimeFiber<void, SqsOutputError> | null>(null)
  const metrics = new MetricsAccumulator("sqs-output")

  const sendBatch = (messages: Message[]): Effect.Effect<void, SqsOutputError> =>
      Effect.gen(function* () {
        if (messages.length === 0) return

        yield* Effect.logDebug(`Sending batch of ${messages.length} messages to SQS`)
        ...
        const batchEffect = Effect.tryPromise({
          try: async () => {
            ...
            if (result.Failed && result.Failed.length > 0) {
              ...
              throw new Error(`Failed to send ${result.Failed.length} messages (IDs: ${failedIds})`)
            }
  ...
  const startBatchTimeout = Effect.gen(function* () {
    ...
    const timeoutEffect = Effect.gen(function* () {
      ...
      yield* Effect.logDebug(`Batch timeout reached (${config.batchTimeout}ms), flushing batch`)
  ...
  return {
    ...
    send: (msg: Message): Effect.Effect<void, SqsOutputError> =>
      Effect.gen(function* () {
        ...
        const newBatch = yield* Ref.updateAndGet(batchRef, (batch) => [...batch, msg])
    ...
    close: () =>
      Effect.gen(function* () {
        ...
        if (remaining.length > 0) {
          yield* sendBatch(remaining).pipe(
