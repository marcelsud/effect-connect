 ...
 * ```
 */
export const createHttpOutput = (config: HttpOutputConfig): Output<HttpOutputError> => {
  ...
  Effect.runSync(Effect.log(`HTTP Output initialized: ${method} ${config.url}`))
    ...
    send: (message: Message): Effect.Effect<void, HttpOutputError, never> =>
      Effect.gen(function* () {
        ...
        const httpRequest = HttpClientRequest.make(method)(config.url).pipe(
          ...
          HttpClientRequest.setHeaders({ "User-Agent": "effect-connect/0.1.1" })
        )
        ...
        const [_, duration] = yield* measureDuration(
          Effect.gen(function* () {
            ...
            if (response.status >= 400) {
              const category = response.status >= 500 ? "intermittent" : "logical"
