...
import { createAssertProcessor, AssertProcessorError } from "../../../src/testing/assert-processor.js"
...
describe("AssertProcessor", () => {
  describe("Field Assertions", () => {
    it("should pass when all required fields exist", async () => {
      ...
      const message = createMessage({ id: "123", value: "test", extra: "data" })
    ...
    it("should fail when required field is missing", async () => {
      ...
      await expect(Effect.runPromise(processor.process(message))).rejects.toThrow(
        "Missing field 'name'"
      )
    })
    ...
    it("should fail on missing nested fields", async () => {
      ...
      await expect(Effect.runPromise(processor.process(message))).rejects.toThrow(
        "Missing field 'user.address.city'"
      )
    })
  ...
  describe("Condition Assertions", () => {
    ...
    it("should fail when condition evaluates to false", async () => {
      ...
      await expect(Effect.runPromise(processor.process(message))).rejects.toThrow(
        "Condition 'content.amount > 50' evaluated to false"
      )
    })
  ...
  describe("Pass-Through Behavior", () => {
    it("should not modify message content", async () => {
      ...
      const originalContent = { id: "123", value: "test", nested: { data: "example" } }
    ...
    it("should preserve message metadata", async () => {
      ...
      const message = createMessage({ value: "test" }, { source: "input", custom: "data" })
